...
const { GET, POST_FORM } = require('./http');
...
function parsePost($post, threadId) {
  try {
    ...
    post.content.text = post.content.text.replace(/<!--.*-->|\t|\n|\r|\\r|\\t|\\n/gi, '');

    // clean again
    post.content.html = post.content.html.trim();
    post.content.text = post.content.text.trim();
...
function parsePosts(tid, response) {
...
function parseSecurityToken(response) {
...
async function getPostList(tid, pageNum = 0) {
  try {
...
async function quickPost(securitytoken, userId, threadId, message) {
